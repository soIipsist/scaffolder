...
from utils.sqlite import select_items, update_item, insert_items, delete_items, get_filter_condition, create_connection, sanitize_filter_condition
import re
...
class SQLiteItem:
    ...
    def __init__(self, table_values:list, column_names:list = None) -> None:
        ...
        self.column_names = column_names if column_names else self.get_column_names_from_table()
    ...
    def select(self):
        items =  select_items(conn, self.table_name, self.filter_condition, type(self), column_names=self.column_names)
    ...
    def import_item(self, path:str, regex:list = [], attr_names:list = [], insert = True):
