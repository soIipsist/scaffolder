...
def get_questions_from_sqlite_object(object, attr_names:list = [], get_empty_attributes:bool=False):
...
def get_question(question_dict:dict, object_key:str = None, object_value:str = None):
    ...
    question_message = question_dict.get("message", object_key if object_key else "message")
    question_type = question_dict.get("type", questionary_actions.get(type(object_value), "text"))
    ...
    question_dict.update({"type" : question_type, "message": question_message, "name": question_name})

    # add choices if 'select' type
    if question_type == 'select' and not 'choices' in question_dict:
        choices_val = object_value if isinstance(object_value, list) and len(object_value) > 0 else ['Select', 'Update']            
...
def prompt_questions(questions:list, object=None, question_names:list=[]):
    ...
    if question_names:
        questions = [question for question in questions if question.get('name') in question_names]
