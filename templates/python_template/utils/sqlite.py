...
def insert_items(conn:sqlite3.Connection, table_name:str, objects:list, column_names:list=None):
    """
    ...
    try:
        cursor = conn.cursor()
        column_names = get_column_names(cursor, table_name) if column_names is None else column_names
        print(table_name)
...
def update_item(conn:sqlite3.Connection, table_name:str, obj:dict, filter_condition:str, updated_columns:list = None):
...
def select_items(conn:sqlite3.Connection, table_name:str, filter_condition:str=None, mapped_object_type=None, column_names:list = []):
    """ 
    ...
    return results if not mapped_object_type else map_sqlite_results_to_objects(results, mapped_object_type, column_names)


def select_random_item(conn:sqlite3.Connection, table_name:str, mapped_object_type=None, column_names:list = []):
    """
    ...
    return results if mapped_object_type is None else map_sqlite_results_to_objects(results, mapped_object_type, column_names)
    

def map_sqlite_results_to_objects(sqlite_results:list, object_type, column_names:list = []):
...
def delete_items(conn:sqlite3.Connection, table_name:str, filter_condition:str="id = ?"):
    """
...
def get_random_row(conn:sqlite3.Connection, table_name:str):
    ...
    # print(table_name)
    return execute_query(conn, "SELECT * FROM {} ORDER BY RANDOM() LIMIT 1".format(table_name))
